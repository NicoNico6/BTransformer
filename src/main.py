...
from models.modules import MultiBConv, BConv, BLinear, GhostSign, GhostBNSign

from models.SwinTransformer import CSWin_64_12211_tiny_224, CSWin_64_24322_small_224, CSWin_96_24322_base_224, CSWin_144_24322_large_224
...
parser.add_argument("--imagenet_directory", type = str, default = "/home/datasets/vision/ImageNet/extracted/", help = "imagenet directory")
...
def get_data(opt):
  if not opt.dataset == 'ImageNet':
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.train_batch_size, shuffle = True, num_workers=opt.num_workers, pin_memory=False)
    testloader = torch.utils.data.DataLoader(testset, batch_size=opt.test_batch_size, shuffle=False, num_workers=opt.num_workers, pin_memory=False)
    ...
    pipe = HybridTrainPipe(batch_size=opt.train_batch_size, num_threads=max(1, opt.num_workers), device_id=int(opt.gpu), data_dir=traindir, crop=crop_size, dali_cpu=opt.dali_cpu)
    pipe.build()
    trainloader = DALIClassificationIterator(pipe, size=int(pipe.epoch_size("Reader") / 1), auto_reset = True)

    pipe = HybridValPipe(batch_size=opt.test_batch_size, num_threads=max(1, opt.num_workers), device_id=int(opt.gpu), data_dir=valdir, crop=crop_size, size=val_size)
    pipe.build()
...
def get_model(opt, num_classes, mode = "student", model = "boolnet18"):
    if mode in {"student", "pretrain"}:
      ...
      if model == 'Tiny':
        ...
        net = CSWin_64_24322_small_224(num_classes = num_classes)

      elif model == 'Base':
          net = CSWin_96_24322_base_224(num_classes = num_classes)

      elif model == 'Large':
          net = CSWin_144_24322_large_224(num_classes = num_classes)
...
def set_temperature_cifar(net, opt, epochs, freq = 5):
    ...
    for i, module in enumerate(net.module.modules() if opt.parallel else net.modules()):
        if isinstance(module, (BConv, MultiBConv, GhostBNSign, GhostSign, BLinear)):
          if epochs != 0:
