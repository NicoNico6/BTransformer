...
from .modules import BLinear, BConv, binarize
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = BLinear(in_features, hidden_features)
        self.act = act_layer()
        self.fc2 = BLinear(hidden_features, out_features)
...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0.,
        ...
        self.W_sp = W_sp
        stride = 1
        self.get_v = BConv(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim)
    ...
    def get_lepe(self, x, func):
        ...
        x_binary = self.binarize(x)
        lepe = func(x_binary)  ### B', C, H', W'
    ...
    def forward(self, qkv):
        ...
        q = self.binarize(self.norm(q))
        k = self.binarize(self.norm(k))
        v = self.binarize(self.norm(v))
        
        attn = (q @ k.transpose(-2, -1))  # B head N C @ B head C N --> B head N N
        #attn = nn.functional.softmax(attn, dim=-1, dtype=attn.dtype)
        attn = self.attn_drop(attn)
        
        attn = self.binarize(attn)
        
        x = self.norm((attn @ v)) + self.norm(lepe)
...
class CSWinBlock(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads,
        ...
        self.qkv = BLinear(dim, dim * 3, bias = qkv_bias)
    ...
    def forward(self, x):
        ...
        attened_x = self.binarize(attened_x)    
...
def CSWin_64_12211_tiny_224(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=32, depth=[1, 2, 21, 1],
...
if __name__ == '__main__':
    test_model = CSWin_64_12211_tiny_224(num_classes = 100, img_size = 224).cuda()
